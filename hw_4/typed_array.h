...
public:
    ...
    //pop push push_front pop_front 
    void push(const ElementType value);
    void push_front(const ElementType value) ;
    ElementType pop() ;
    ElementType pop_front() ;
    //other method 
    TypedArray concat(const TypedArray& source);
    TypedArray& reverse();
private:
...
//concat
template <typename ElementType>
TypedArray<ElementType> TypedArray<ElementType>::concat(const TypedArray<ElementType>& source) {
    TypedArray<ElementType> res = *this;
    for (int i=0;i<source.size();i++){
        res.push(source.safe_get(i));
    }
    return res;
}
// +operation 
template <typename ElementType>
TypedArray<ElementType> TypedArray<ElementType>::operator +(const TypedArray<ElementType>& other) {
    TypedArray<ElementType> temp = *this;
    TypedArray<ElementType> res = temp.concat(other);
    return res;
}
// Pop
template <typename ElementType>
ElementType TypedArray<ElementType>::pop() {
    ...
    set(size()-1, x);    // `set` will create a copy of the default values
    return value;
}
//pop_front
template <typename ElementType>
